...
const Preview = ({
  ...
  return (
   ...
   {!isLoading  ? (<div>
      {url ? (<div className="flex flex-col gap-4">
        <div className="flex gap-10 items-center">
          <div>
            <img className="w-28 rounded-full" src={avatar_url} alt="image" />
          </div>
          <div>
            <h1>{name}</h1>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-dark-bg p-3 gap-10 rounded-md text-blue-500">Profile URL: <a href={url} className="text-white">{url}</a></div>
        </div>
      </div>):(<div>No user found. Search with the registered Username</div>) }
      </div>
